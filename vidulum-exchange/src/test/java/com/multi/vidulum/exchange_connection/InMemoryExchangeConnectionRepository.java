...
import com.multi.vidulum.exchange_connection.infrastructure.ExchangeConnectionEntity;
 ...
 *
 * <ul>
 *   <li>the natural key {@code (userId, broker, environment, accountUid)} is unique, and a
...
class InMemoryExchangeConnectionRepository implements DomainExchangeConnectionRepository {
    ...
    public Optional<ExchangeConnection> findByAccount(
        ...
        return store.values().stream()
                ...
                .filter(entity -> entity.getBroker().equals(broker.getId()))
    ...
    private static boolean sharesNaturalKey(
        ...
        return left.getUserId().equals(right.getUserId())
                && left.getBroker().equals(right.getBroker())
