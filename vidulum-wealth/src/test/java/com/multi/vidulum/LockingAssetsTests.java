...
class LockingAssetsTests extends WealthIntegrationTest {
    ...
    void shouldLockCash() {
        ...
        WealthIntegrationTest.TestUserSummary createdUser = createUser("lu92", "secret12", "lu92@email.com");

        activateUser(createdUser.userId());

        WealthIntegrationTest.TestPortfolioSummary registeredPortfolio = registerPortfolio("XYZ", "BINANCE", createdUser.userId(), "USD");

        depositMoney(PortfolioId.of(registeredPortfolio.portfolioId()), Money.of(100000.0, "USD"));
        ...
        Awaitility.await().atMost(10, SECONDS).until(() -> {
            Portfolio portfolio = portfolioRepository.findById(PortfolioId.of(registeredPortfolio.portfolioId())).get();
