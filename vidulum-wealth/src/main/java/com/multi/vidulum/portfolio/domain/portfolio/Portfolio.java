...
public class Portfolio implements Aggregate<PortfolioId, PortfolioSnapshot> {
    ...
    public void depositMoney(Money deposit, String contributionId, ZonedDateTime when) {
        MoneyDepositedEvent event = new MoneyDepositedEvent(portfolioId, deposit, contributionId, when);
        apply(event);
        add(event);
    }

    public void withdrawMoney(Money withdrawal, String contributionId, ZonedDateTime when) {
