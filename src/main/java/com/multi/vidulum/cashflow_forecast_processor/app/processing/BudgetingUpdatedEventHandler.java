...
@Component
@AllArgsConstructor
public class BudgetingUpdatedEventHandler implements CashFlowEventHandler<CashFlowEvent.BudgetingUpdatedEvent> {

    private final CashFlowForecastStatementRepository statementRepository;

    @Override
    public void handle(CashFlowEvent.BudgetingUpdatedEvent event) {
        ...
        if (categoryNode != null && categoryNode.getBudgeting() != null) {
            Budgeting updatedBudgeting = categoryNode.getBudgeting().withUpdatedBudget(event.newBudget(), event.updated());
            categoryNode.setBudgeting(updatedBudgeting);
        }
        ...
        statement.getForecasts().values().forEach(cashFlowMonthlyForecast -> {
            ...
            if (cashCategory != null && cashCategory.getBudgeting() != null) {
                Budgeting updatedBudgeting = cashCategory.getBudgeting().withUpdatedBudget(event.newBudget(), event.updated());
                cashCategory.setBudgeting(updatedBudgeting);
            }
        });
