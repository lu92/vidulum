...
/**
 * Handler for CashFlowActivatedEvent.
 * Changes all IMPORT_PENDING months to IMPORTED status.
 */
@Slf4j
@Component
@AllArgsConstructor
public class CashFlowActivatedEventHandler implements CashFlowEventHandler<CashFlowEvent.CashFlowActivatedEvent> {

    private final CashFlowForecastStatementRepository statementRepository;

    @Override
    public void handle(CashFlowEvent.CashFlowActivatedEvent event) {
        ...
        log.info("CashFlow [{}] activated. Changed [{}] months from IMPORT_PENDING to IMPORTED. " +
