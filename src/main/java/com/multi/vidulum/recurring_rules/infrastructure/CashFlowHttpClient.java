...
public class CashFlowHttpClient {
    ...
    /**
     * Status information for a cash change.
     */
    public record CashChangeStatusInfo(
    ...
    /**
     * Gets the status information for all cash changes in a CashFlow that were generated by a specific rule.
     *
     * @param cashFlowId   the CashFlow ID
     * @param sourceRuleId the recurring rule ID to filter by
     * @param authToken    the auth token
     * @return map of CashChangeId to CashChangeStatusInfo
     */
    public Map<CashChangeId, CashChangeStatusInfo> getCashChangeStatuses(
            CashFlowId cashFlowId,
            RecurringRuleId sourceRuleId,
            String authToken
    ) throws CashFlowNotFoundException, CashFlowCommunicationException {
        ...
        try {
            ...
            Map<String, Object> body = response.getBody();
            if (body == null) {
                throw new CashFlowCommunicationException(cashFlowId, "get cash change statuses", "Empty response body");
            }

            @SuppressWarnings("unchecked")
            Map<String, Map<String, Object>> cashChanges = (Map<String, Map<String, Object>>) body.get("cashChanges");

            if (cashChanges == null) {
                return Map.of();
            }

            Map<CashChangeId, CashChangeStatusInfo> result = new HashMap<>();
            for (Map.Entry<String, Map<String, Object>> entry : cashChanges.entrySet()) {
                String ccId = entry.getKey();
                Map<String, Object> cc = entry.getValue();
                String ccSourceRuleId = (String) cc.get("sourceRuleId");

                // Filter by sourceRuleId if provided
                if (sourceRuleId != null && !sourceRuleId.id().equals(ccSourceRuleId)) {
                    continue;
                }

                String status = (String) cc.get("status");
                CashChangeStatusInfo statusInfo = new CashChangeStatusInfo(
                        new CashChangeId(ccId),
                        status,
                        ccSourceRuleId
                );
                result.put(new CashChangeId(ccId), statusInfo);
            }

            return result;
        } catch (HttpClientErrorException.NotFound e) {
            throw new CashFlowNotFoundException(cashFlowId);
        } catch (CashFlowNotFoundException | CashFlowCommunicationException e) {
            throw e;
        } catch (Exception e) {
            throw new CashFlowCommunicationException(cashFlowId, "get cash change statuses", e);
        }
    }
}
