...
| ~~`Portfolio.investedBalance`~~ | **zamknięte (C9, C12).** Pole zastąpione listą wkładów; ścieżka spec-u zapisuje wkład otwarcia, więc portfel ze snapshotu nie raportuje już zera | — |
...
| B4 | Łańcuch denominacji do PLN | `openAvgPx` jest w USD niezależnie od pary; OKX nie ma par PLN. Potrzebny kurs USD/PLN z NBP i rozszerzenie fallbacku (dziś tylko `X/USD → X/USDT` z założeniem 1:1). **Po E10 to już nie jest wyłącznie sprawa par PLN.** Snapshot obejmuje teraz konto Funding, więc walor, którego OKX nie notuje, trafia do portfela — a `GET /portfolio` wycenia **cały** portfel, więc brak jednego notowania rzuca `QuoteNotFoundException` na całość i czyni portfel nieodczytywalnym. Przed E10 taki walor nie trafiał do snapshotu i nie mógł zaszkodzić. B4 przestaje być poprawnością długoterminową, a staje się odpornością odczytu. | P2 | B3 |
...
| C5 | Zmiana wartości majątku | Osobna miara, **niewymagająca ceny nabycia** — odpowiada na „o ile zmienił się mój majątek", gdzie część nieznana jest pełnoprawna. **Zrobione**: `wealthChange` = `currentValue − netContributions`, plus `pctWealthChange`. Liczona **od początku**, nie w oknie — każdy wpis w księdze ma datę, ale nie ma zapisanych wycen historycznych, więc okno („ten miesiąc") wymaga dopiero historii PnL; obiecywanie go teraz byłoby liczbą bez pokrycia. Nie ma własnego statusu: milknie dokładnie wtedy, gdy milknie `netContributions`, a enum, który nigdy nie mógłby się różnić od `ContributionStatus`, byłby drugą nazwą na ten sam fakt. Procent wstrzymany przy wkładzie ≤ 0 — kto wyjął więcej, niż włożył, nie ma bazy, a dzielenie przez ujemną odwraca znak bez odwrócenia sensu. Na żywym koncie to **jedyna** liczba, która może mówić: wynik jest wstrzymany przy 8% pokrycia kosztem, a tu pokrycie bierze się z księgi, nie z cen nabycia. | P1 | C2, C9 |
| C6 | Naprawa `AggregatedPortfolio` | Scalanie po `(ticker, subName)` zamiast po samym `ticker`, inaczej widok zbiorczy rozcieńcza średnią. **Zrobione**: klucz `PositionKey(ticker, subName)` w obu miejscach, które scalały — `mergeAssetsAtSamePosition` i `findRelatedAsset`. Stary kod dodatkowo stemplował scaloną pozycję `SubName.none()`, więc w wyniku nie zostawał ślad, że cokolwiek scalono: widok zbiorczy pokazywał jedną linię BTC ze średnią ceną rozciągniętą na ilość, której ta cena nigdy nie dotyczyła. To był dokładnie ten podział, który wprowadził C2 — kupione tutaj kontra przyniesione z zewnątrz bez ceny — odbudowany o poziom wyżej. | P1 | C2 |
| C7 | Reguła sprzedaży nieznanej części | Sprzedaż pozycji bez kosztu to zdarzenie podatkowe, którego nie policzymy. Zażądać ceny albo zapisać z jawnie brakującym kosztem — nigdy nie przyjmować zera. **Zrobione razem z F6**, bo to jedna ścieżka kodu: sprzedaż zapisuje się zawsze, z przychodem, który jest znany, i wynikiem, który bywa `null`. Zero oznaczałoby „cały przychód jest zyskiem" — nieprawda, a przy rozliczeniu nieprawda **na niekorzyść** właściciela. Obsłużony jest też przypadek pośredni, po D6 zwyczajny: pozycja większa niż to, co umiemy wycenić. Wynik liczy się wtedy z części pokrytej i mówi, jaka to była część, zamiast rozciągać się na jednostki, których nigdy nie opisywał. | P2 | C2 |
...
| G6 | Pozostałe wyjątki portfela zwracają 500 zamiast swoich kodów | `PortfolioNotFoundException` dziedziczyło po `RuntimeException`, więc odmowa własności wracała jako `500` — nie do odróżnienia od awarii. Naprawione przy G2 — **to zadanie obejmuje już tylko resztę**. **Zrobione**: wszystkie sześć dziedziczy teraz po `BusinessException` i ma własny `ErrorCode` — „nie masz takiej pozycji" (404), „nie masz tyle" (422), „portfel zamknięty" (409), „tych jednostek nie da się zwolnić" (422), „ten ticker masz w kilku pozycjach, powiedz w której" (422), „taka pozycja już jest" (409). Handler łapie `BusinessException` ogólnie, więc mapowanie wchodzi samo. Pokryte testem **endpointowym**, bo to jedyny poziom, na którym różnica 500 kontra odmowa w ogóle istnieje — w procesie każde z nich jest po prostu wyjątkiem. Pilność wzrosła przy C8: dołożyliśmy endpoint, który rzuca `AssetNotFoundException`, czyli **nową ścieżkę do istniejącej dziury**. Sześć pozostałych w tym samym pakiecie miało ten sam problem: `AssetNotFoundException`, `NotSufficientBalance`, `PortfolioIsNotOpenedException`, `CannotUnlockAssetException`, `AmbiguousAssetSelectionException`, `DuplicateAssetPositionException`. Każdy wymaga decyzji o statusie HTTP, więc nie doklejam ich do zadania o własności. To ta sama klasa, którą A3 zamknęło dla modułu giełdowego. **Potwierdzone na żywym backendzie**: wycena nieistniejącej pozycji → `404 PORTFOLIO_ASSET_NOT_FOUND`, wypłata 9 999 przy stanie 1 000 → `422 PORTFOLIO_INSUFFICIENT_BALANCE`, zwolnienie blokady, której nie ma → `422 PORTFOLIO_CANNOT_UNLOCK` — każde z komunikatem biznesowym zamiast 500. | P2 | — |
...
| C12 | Wkład otwarcia przy `confirm` | Krok 2 projektu wkładów. Portfel ze snapshotu dostaje **jeden** wkład otwarcia: „na ten dzień konto było warte X”. Ma **strukturalną konsekwencję** — `ConfirmPortfolioSpecCommandHandler` musi zacząć czytać notowania, bo dziś tworzy portfel nie znając cen, a wycena dzieje się dopiero przy `GET /portfolio`. Obejścia nie ma: doliczenie wartości przy pierwszym odczycie dałoby cenę z chwili zajrzenia, nie z chwili przyjścia. Dane już tam są — E8 wymusiło publikację kursów przed onboardingiem — nowa jest zależność, nie dane. Zmienia się **moment awarii**: brak kursu wywali `confirm` zamiast pierwszego odczytu, co uznajemy za poprawę (lepiej nie utworzyć portfela, niż utworzyć nieczytelny). Własna proweniencja `OPENING_SNAPSHOT`: giełda nie powiedziała „to twój wkład”, tylko „tyle masz”, a ta różnica musi być widoczna w danych. **Zrobione**: `ConfirmPortfolioSpecCommandHandler` dostał `QuoteRestClient` i wycenia **pozycja po pozycji**, nie jedną kwotą — jeden nienotowany walor kosztuje wtedy jedną linię, a nie całą liczbę, z tego samego powodu, dla którego projekt wybrał listę zamiast skalara. Wpis niesie id, bo backfill (C13) ma go kiedyś zastąpić. | P1 | C9, D3 |
| F7 | Typowane `ContributionId` i zegar na krawędzi | **Zrobione przy okazji C9/C12.** Dwie rzeczy, które nowy kod wniósł w złej postaci. Po pierwsze id wkładu było `String`, a ścieżka wpłaty przekazuje obok siebie id wkładu, id portfela i kod waluty — trzy łańcuchy, których zamianę kompilator przyjąłby bez słowa; `ContributionId` stoi przy `Contribution`, nie w `shared-kernel`, bo wkład nie jest agregatem, który ktokolwiek z zewnątrz adresuje (precedens: `ExchangeConnectionId`). Po drugie `Clock` siedział w handlerach `deposit`, `withdraw` i `confirm`: ta sama komenda wykonana dwa razy zapisywała inny wpis, a test, który chciał ustalonego momentu, musiał wstrzykiwać zegar. Zegar przeszedł do kontrolerów, komenda niesie `dateTime` (nazwa zgodna ze zdarzeniami) i mówi cały fakt. Przy `confirm` argument jest mocniejszy niż przy wpłacie: zatwierdzenie datuje **trzy** fakty — wkład otwarcia, `markApplied`, `markStale` — i wszystkie muszą nieść ten sam moment, czego zegar odpytywany per użycie nie gwarantuje. | P3 | C9, C12 |
| F8 | Reszta handlerów wciąż trzyma `Clock` | F7 zrobiło to dla trzech handlerów z tego brancha; pozostałe **34** (cashflow, exchange, `CreatePortfolioSpecCommandHandler`, `LockAssetCommandHandler`, PnL) nadal czytają czas w środku. Nie jest to błąd, tylko niespójność: dwie konwencje obok siebie są gorsze niż jedna zła, bo przy każdym nowym handlerze trzeba zgadywać, którą wybrać. Świadomie **nie** robione razem z C9/C12 — to zmiana mechaniczna w kilkudziesięciu plikach i utopiłaby przegląd zmiany merytorycznej. | P3 | F7 |
| C15 | `AssetSummaryJson` nie mówi, która to pozycja | Znalezione przy dowodzeniu D13 na żywym backendzie: `GET /portfolio/{id}/{currency}` zwraca **dwa wiersze BTC** — jeden kupiony, jeden przyniesiony z zewnątrz — i **żaden z nich nie niesie `subName`**. Klient widzi dwie pozycje o tym samym tickerze i nie ma czym ich rozróżnić; skrypt dowodowy musiał je rozpoznawać po tym, czy mają koszt, co jest zgadywaniem, nie odczytem. Cały podział wprowadzony w C2 istnieje w modelu i w bazie, ale urywa się na granicy API. Naprawa jest jednolinijkowa w DTO; kosztem będą asercje całoobiektowe, które słusznie się wywalą. **Zrobione**: `AssetSummaryJson.subName`. Gotówka jest nazywana `none`, nie pustym łańcuchem — `SubName.none().getName()` zwraca `""`, co każdy interfejs pokaże jako pustą komórkę nie do odróżnienia od pola, którego nikt nie wypełnił; wiersz najtrudniejszy do odgadnięcia nie może czytać się jako brakujący. Wywaliło 13 asercji całoobiektowych w trzech klasach (dokładnie po to są) i odsłoniło fikstury budujące `Asset` bez `subName` — czego produkcja nie robi nigdy. | P1 | C2 |
| C14 | Zmiana majątku w oknie czasowym | C5 liczy **od początku** istnienia portfela, bo tyle da się powiedzieć uczciwie bez zapisanych wycen. Pytanie, które użytkownik zada jako drugie, brzmi „a w tym miesiącu?" — i wymaga punktu odniesienia z konkretnej daty. Wzór zostaje ten sam: `wycena(koniec) − wycena(początek) − wkłady w oknie`; wpisy w księdze są datowane, więc człon odejmowany jest gotowy. Brakuje wyłącznie `wycena(początek)`, czyli F9. Do rozstrzygnięcia produktowego: które okna (miesiąc, rok, od onboardingu) i co pokazać, gdy portfel jest młodszy niż okno. **Zrobione**: `GET /pnl/wealth-change?portfolioId=…&since=…`. Okno nie wymagało **żadnych nowych danych** — wpis wyceny niesie też wkłady na tamten dzień, więc wpłaty wewnątrz okna to różnica dwóch liczb, a nie przejście po księdze. Odpowiedź zawsze niesie status i **dzień, który faktycznie odpowiedział**: zapis dzienny rzadko trafia w pytaną chwilę, a czytelnik ma prawo wiedzieć, co dostał. Dwie ciszę: `NOT_WATCHING_YET`, gdy okno zaczyna się przed pierwszym zapisem, i `CONTRIBUTIONS_UNKNOWN`, gdy któryś koniec nie wie, ile wpłacono — bez tego odjęcia przelew czyta się jako zysk, czyli dokładnie błąd, przed którym broni C5. **Potwierdzone na żywym backendzie**: wpłata 20 000 USD, 5 XAU po 2 000, zapis wyceny z harmonogramu, potem kurs 2 400 i dopłata 5 000 — `GET /pnl/wealth-change` odpowiada `COMPUTED` i `2 000 USD`, czyli `(27 000 − 20 000) − (25 000 − 20 000)`: dopłata nie udaje zysku. `measuredFrom` wskazał zapis o trzy sekundy wcześniejszy niż pytanie, więc reguła „najbliższy zapis w tył” widać w samej odpowiedzi; okno zaczynające się przed pierwszym zapisem wraca `NOT_WATCHING_YET` z pustą zmianą, nie zerem. | P2 | C5, F9 |
| F9 | Historia wycen portfela (PnL) jako źródło punktu odniesienia | Moduł `pnl` ma już `PnlHistory` i `MakePnlSnapshotCommandHandler`, ale nic nie robi ich regularnie ani nie czyta ich pod wycenę historyczną. Bez tego każda miara „w oknie" musiałaby zgadywać wartość z przeszłości z dzisiejszych cen — czyli podstawiać dzisiejszą cenę pod wczorajszy stan, co jest tą samą klasą kłamstwa co zerowy `investedBalance`. **Zrobione.** Okazało się, że migawka zapisuje **i wartość, i wkłady** — brakowało wyłącznie dwóch rzeczy: regularności i odczytu po dacie. `PnlHistory.valuationAt(portfel, moment)` zwraca **ostatni zapis nie późniejszy** niż pytanie (nigdy późniejszy — to odpowiadałoby wiedzą, której ta data nie miała) i pusty wynik, gdy nic wcześniej nie zapisano; absencja jest tu stanem zwyczajnym, nie błędem. `DailyValuationScheduler` zapisuje wyceny raz na dobę, z awarią **per właściciel**: brak kursu u jednego nie może kosztować wszystkich pozostałych zapisu, bo luka, którą zostawia, jest nieodwracalna — ceny waloru z minionego wtorku nie da się dobrać później. **Potwierdzone na żywym backendzie**: harmonogram rusza w kontenerze i zapisuje (`Recorded 1 of 1 valuation(s)`), a na pustej bazie melduje `0 owner(s)` — historia powstaje dopiero przy rejestracji użytkownika, więc zero jest stanem, nie cichą awarią. Próba nie wymagała ruszania repozytorium: cron jest konfigurowalny (`${vidulum.valuation.cron:0 0 1 * * *}`), więc wystarczyło nadpisać go na dziesięć sekund w uruchomieniu. | P2 | — |
...
| C9 | Model wkładów i jeden mechanizm | **Odłożone. Decyzja POC: zostaje zerem.** Pole aktualizują wyłącznie `deposit` i `withdraw`, a ścieżka spec-u omija obie — portfel ze snapshotu pokaże „zainwestowano 0" przy sześciu aktywach. Świadomie odłożone, **nie przeoczone**. Do rozstrzygnięcia: czy ma to być suma znanych kosztów przeliczona na walutę wyceny, czy pole traci sens przy portfelu ze snapshotu i wymaga zastąpienia miarą liczoną z `CostBasis`. Interfejs nie może pokazywać zera jako prawdy. **Rozstrzygnięte** — projekt w [2026-09-23-invested-balance-as-contributions.md](2026-09-23-invested-balance-as-contributions.md). Pole zastępuje **lista wkładów**, a `investedBalance` staje się odczytem wyprowadzonym z pokryciem i statusem (wzorzec C4). Decyzja prowadząca: **wkład do portfela to nie koszt nabycia** — mierzą od różnych granic, więc muszą stać obok siebie i nigdy się nie mieszać. **Zrobione** (krok 1): `Portfolio` trzyma `List<Contribution>` zamiast `Money investedBalance`; `deposit` i `withdraw` dopisują do tej samej księgi (jeden mechanizm), a id wpisu i moment wchodzą **z zewnątrz** — przez komendę i zdarzenie — żeby agregat pozostał deterministyczny i żeby test mógł je przypiąć. Odczyt (`netContributions`, `contributionCoverage`, `contributionStatus`) powtarza regułę C4 o jeden poziom niżej: suma policzona z mniejszości wpisów jest **wstrzymywana**, nie opatrywana przypisem. Cztery statusy zamiast `null`: `COMPUTED`, `WITHHELD_LOW_COVERAGE`, `NO_KNOWN_VALUE`, `NOTHING_CONTRIBUTED` — „nic nie wpłacono” i „nie wiemy, ile to było warte” to różne zdania. Wkład zapisuje się w walucie, w której przyszedł, i jest przeliczany dopiero przy odczycie — pominięcie tego przeliczenia kazało 10 000 USD raportować jako „10 000 EUR”, co złapały testy. Kroki dalsze: C5, C13. | P1 | C1, D3 |
| C8 | „Nieznane" jako zadanie | Ekran/flaga „uzupełnij cenę nabycia". Bez tego użytkownik nie odliczy kosztów przy PIT. **Zrobione.** Dwie rzeczy: flaga `awaitingCost` w każdym wierszu odczytu — wyprowadzona z pokrycia, ale **podana wprost**, żeby interfejs miał zadanie do pokazania zamiast liczby do interpretacji — oraz `POST /portfolio/asset/cost`, którym właściciel podaje cenę. Do tej pory dało się to zrobić **wyłącznie w trakcie onboardingu**, przy otwartym spec-u; pozycja, która przyszła później, nie miała żadnej drogi i jej wynik zostawał wstrzymany na zawsze. Cena pokrywa to, co jest trzymane **teraz**; jednostki, które dojdą później, nie są objęte starszą odpowiedzią — pokrycie spada, zamiast cena rozciągać się na to, czego nigdy nie opisywała (C4, D6). Zapis idzie jako `USER_PROVIDED`, czyli jedyna proweniencja, której synchronizacja nie nadpisze po cichu — i to jest warunek sensowności całego pytania: odpowiedź, którą następny odczyt z giełdy by skasował, nie jest warta zadawania. Wskazanie pozycji, której nie ma, jest odrzucane, a nie tworzy pozycji z odpowiedzi. Bazuje na C15: bez `subName` w odczycie nie dałoby się powiedzieć, **której** pozycji dotyczy cena. **Zakres: backend.** Ekran należy do interfejsu; tutaj jest to, na czym może stanąć — flaga w odczycie i endpoint do odpowiedzi. **Potwierdzone na prawdziwym koncie OKX**: system sam wskazał trzy pozycje bez ceny (BTC, ETH, XRP — wszystko, co przyszło przelewem), gotówka nie pytała o nic, a po podaniu 25 000 EUR bitcoin zyskał koszt `USER_PROVIDED`, przestał czekać i pokazał wynik 48 892 EUR; pokrycie portfela wzrosło. | P2 | C2, C15 |
...
| D5 | Odwzorowanie locków | `frozenBal` → `Asset.locked`, `availBal` → `Asset.free`. Rozstrzygane automatycznie, bez pytania — otwarte zlecenie to fakt, nie decyzja, a jedyne pytanie, jakie mogłoby postawić („czy to naprawdę zablokowane?"), ma jedną odpowiedź. **Po E10 oba pola są po dwóch stronach** — Trading i Funding mają własne `frozenBal`, więc odwzorowanie je sumuje: właściciel ma jeden bilans, nie dwa. **Zrobione**: `SnapshotPosition` niesie `frozen` (≤ `total`, brak czyta się jako zero), `ConfirmPortfolioSpecCommandHandler` rozdziela je na pozycje. **Rozdział jest regułą, nie faktem**: giełda zamraża *walutę*, a my trzymamy pozycje rozbite po pochodzeniu (C2), i nic tych dwóch rzeczy nie łączy — jednostki są zamienne, więc nie istnieje fakt mówiący, który bitcoin zablokowało zlecenie. Reguła: najpierw część handlowana, reszta przelewa się na przyniesioną z zewnątrz. Dokładna jest **suma** — a to ona odpowiada na pytanie, które właściciel faktycznie zadaje. `activeLocks` zostaje puste: lock tam jest *nasz*, trzymany przeciw naszemu zleceniu i zwalniany odblokowaniem; wymyślenie lokalnego `orderId` dla cudzej blokady stworzyłoby lock, którego nic nigdy nie zwolni. Potwierdzone na żywym koncie: `frozenBal` 0,325 BTC rozłożone jako 0,00327 (handlowana) + 0,32173 (przyniesiona) i 240 EUR na gotówce. | P1 | D3, E10 |
| D6 | Reguły nadpisywania wg proweniencji | `ASSUMED_PAR` i `EXCHANGE_REPORTED` nadpisywalne po cichu, `USER_PROVIDED` **nigdy bez pytania** — snapshot nie wie, ile zapłaciłeś na innej giełdzie, więc nadpisanie niszczy jedyne źródło tego faktu. `DERIVED_FROM_FILLS` reguły nie ma i to jest luka: nasze fille są dokładniejsze niż krocząca średnia giełdy. **Dochodzi reguła ciszy.** Dzisiejsze sformułowanie mówi o zastąpieniu jednej wartości drugą, a przypadek z Funding (E10) jest inny — snapshot **nie mówi nic**, bo Funding nie handluje i nie ma `openAvgPx`. Milczenie nie jest wypowiedzią: *synchronizacja, która widzi mniej niż poprzednia, nie ma prawa zapominać*. Bez tego każdy ponowny odczyt waloru leżącego w Funding kasowałby jego koszt. Dziś istnieje jedynie `CostBasis.strongerProtection` — ochrona `USER_PROVIDED` przy scalaniu **dwóch znanych** kosztów, i nic ponadto. **Zrobione**: `Provenance.authority()` porządkuje źródła (użytkownik 3 → własne fille 2 → reszta 1), a `CostReconciliation.afterSynchronisation` stosuje dwie reguły. Pierwsza to **reguła ciszy**: brak ceny w nowym odczycie zostawia znany koszt nietknięty — to przypadek z Funding, gdzie po prostu nie ma `openAvgPx`, a odczytanie tego jako „koszt nieznany" kasowałoby wiedzę i portfel przestawałby raportować wynik po samym przeniesieniu monety między kontami tej samej giełdy. Druga to **autorytet**: nowe jednostki są *wchłaniane* (średnia ważona, bo nic nie jest zastępowane), ale wynik zachowuje **silniejszą** proweniencję — koszt wpisany przez użytkownika po scaleniu z giełdowym nadal jest `USER_PROVIDED`, więc następna synchronizacja nie dokończy nadpisywania po cichu; `DERIVED_FROM_FILLS` nie schodzi do `EXCHANGE_REPORTED`, bo nasze fille to pojedyncze transakcje, z których giełdowa średnia dopiero powstaje. | P1 | D2 |
| D7 | Obsługa zestarzałego snapshotu | Stan `stale`: przeliczyć różnicę od nowa, zadać tylko pytania nadal aktualne, nie stosować nieaktualnych odpowiedzi. **Zrobione razem z D10** — to ten sam mechanizm: `markStale` przelicza wobec stanu znanego, `carryOverAnswers` przenosi decyzje partii, które przetrwały, a `applicable` **odrzuca** odpowiedź nieprzystającą do nowego pytania zamiast wciskać ją na siłę. Status `STALE` ustawia odmowa odpowiedzi na wygasłej kotwicy. Pokryte w `SpecLifecycleComponentTest` i przez endpointy w `PortfolioSpecLifecycleEndpointTest`. Zamknięte jako duplikat zakresu, nie jako osobna praca. | P2 | D1, D10 |
| D8 | Idempotencja i brak pustych spec-ów | Synchronizacja bez zmian **nie tworzy spec-u**. Powtórne zatwierdzenie tego samego spec-u nie zmienia danych. **Zrobione.** Pierwsza połowa istniała i była pokryta (`NothingToSynchroniseException`, potwierdzona też na żywo — 409). Druga miała kod, ale **żadnego testu**: strażnik, którego nikt nie sprawdza, cicho przestaje działać przy pierwszej refaktoryzacji, a ja sam go ruszałem przy D13 i nic by się nie zapaliło. Przy okazji wyszło, że **kłamał w nazwie**: `CannotApplySpecToExistingPortfolioException` mówił, że zatwierdzanie wobec istniejącego portfela jest niewspierane, co D13 uczyniło nieprawdą — a warunek `spec.getPortfolioId() != null` przez cały czas pilnował czegoś innego, bo to pole ustawia `markApplied`. Przemianowany na `SpecificationAlreadyAppliedException` z własnym kodem `PORTFOLIO_SPEC_ALREADY_APPLIED`, plus testy na dwóch poziomach: komponentowy (drugie zatwierdzenie nie rusza pozycji, spec nadal wskazuje ten sam portfel) i endpointowy (409 z kodem, na który klient może się rozgałęzić). **Potwierdzone na żywym backendzie**: powtórne zatwierdzenie odpowiada `409 PORTFOLIO_SPEC_ALREADY_APPLIED`, a właściciel nadal ma jeden portfel. | P2 | D3 |
| D10 | TTL i stany terminalne | TTL 15 min na snapshot, **nie na odpowiedzi**. `stale` przelicza różnicę i zachowuje pasujące odpowiedzi. `cancelled` jako świadome przerwanie onboardingu. `failed` przy nieudanym zapisie. **Zrobione**, i dwie rzeczy okazały się większe, niż wyglądały. Po pierwsze `markStale` **gubił odpowiedzi** — przeliczał różnice od zera, więc użytkownik dostawał ten sam komplet pytań po każdym odświeżeniu; teraz odpowiedź wędruje do różnicy o tej samej partii (ticker, pozycja, ilość), a odpowiedź nieprzystająca do nowego pytania jest odrzucana, nie wciskana na siłę. Po drugie `snapshot` było polem `final`, więc przeliczenie zostawiało **stary** punkt odniesienia i spec, który raz się zestarzał, był stary na zawsze; teraz kotwica jest podmieniana i zegar rusza od nowa. TTL jest **egzekwowany przy odpowiedzi**, nie dopiero przy zatwierdzeniu: `confirm` i tak by odmówił po porównaniu ze świeżym snapshotem (D11), ale wtedy użytkownik ma już odpowiedzianych dwanaście pytań — strata na początku jest odwracalna, na końcu kosztuje porzucony onboarding. Spec przechodzi wtedy w `STALE`, co mówi interfejsowi „zaproponuj ponowny odczyt", a nie „pokaż ten sam formularz". Doszedł `POST /portfolio-spec/{id}/cancel` (cudzy spec odpowiada 404, jak wszędzie), a nieudany zapis portfela ustawia `FAILED` i zachowuje decyzje — „spróbuj ponownie" to inna instrukcja niż „zacznij od nowa". Status po przeliczeniu z zachowanymi odpowiedziami to `CONFIRMED`, nie `DRAFT`: „świeżo policzone, nie ma o co pytać" to nieprawda o spec-ie, który niesie czyjeś decyzje. | P1 | D1, D2 |
| D11 | `confirm` zawsze na świeżym snapshocie | Niezależnie od wieku spec-u: pobierz ponownie, porównaj, przy zgodności zastosuj, przy różnicy wróć do `awaiting_answer`. **Zrobione**. Porównanie i powrót istniały od D3; brakowało drugiej połowy, którą dostarczyło dopiero D13: przeliczenie szło z `List.of()` jako stanem znanym, co dla spec-u zbudowanego wobec istniejącego portfela restytuowało **całe konto jako nowe**. Teraz przelicza się wobec portfela, z którego spec powstał. | P1 | D3, D13 |
| D9 | `DataCleaner` dla `PortfolioSpec` | Wymóg z `CLAUDE.md` dla każdej nowej encji `@Document`. | P1 | D1 |
| D13 | Druga synchronizacja tworzy drugi portfel | `CannotApplySpecToExistingPortfolioException` ma javadoc mówiący, że strzeże zastosowania spec-u do **istniejącego** portfela. Warunek brzmi jednak `spec.getPortfolioId() != null`, a to pole ustawia dopiero `markApplied` — strzeże więc **ponownego zastosowania tego samego spec-u**, nie tego, co obiecuje nazwa. Spec zbudowany przeciwko istniejącemu portfelowi w ogóle nie pamięta, którego: `CreatePortfolioSpecCommand.portfolioId` służy do odczytania stanu znanego i nigdzie się nie zapisuje. Taki spec przechodzi przez kontrolę, a `confirm` buduje **nowy** portfel zawierający same różnice. Dziś ratuje nas kontrola statusu połączenia z D12 — przy spec-u bez połączenia nie ratuje nic. **Zrobione — oba naraz, bo jedno bez drugiego nie zamyka sprawy.** Spec pamięta `knownPortfolioId` (osobne pole od `portfolioId`, i ta różnica jest całym błędem: tamto odpowiada „czy zastosowano", to — „względem czego mierzono"). `confirm` rozgałęzia się: bez portfela tworzy nowy, z portfelem **aktualizuje** ten, względem którego liczył. Aktualizacja idzie przez agregat (`Portfolio.synchronisePosition`), koszty przez D6, blokady przez `ExchangeFreeze` (D5) — restytuowane przy każdym odczycie, bo blokada to fakt o teraz. Wkładu otwarcia **nie** dopisuje się po raz drugi: stałby się twierdzeniem, że właściciel wpłacił ponownie to, co tylko trzymał, a zmiana majątku (C5) zapadłaby się do zera. Połączenie przy ponownym odczycie musi być `ACTIVE` i wskazywać na ten portfel — wskazanie gdzie indziej to pomyłka, nie ponowny odczyt. Zmniejszenia obsłużone jako ujemna delta: pozycja się kurczy, koszt zachowuje cenę i zawęża pokrycie, **żaden wynik zrealizowany nie jest wyliczany** — sprzedaż części o nieznanym koszcie to osobne pytanie (C7). Zejście poniżej zera jest odrzucane (`ImpossibleSynchronisationException`, 409), nie przycinane: przycięcie zostawiłoby portfel niezgodny i z giełdą, i z własną historią. | P1 | D3 |
...
| F6 | Wynik zrealizowany zniknął z podsumowania | C3 zamieniło `profit` na `unrealisedProfit` liczony z pozycji, przez co zysk ze **sprzedanych** pozycji przestał być gdziekolwiek raportowany w podsumowaniu portfela — dawna formuła `currentValue - investedBalance` niosła go mimochodem. Widać to w `TradingPortfolioIntegrationTest`: portfel, który kupił i sprzedał BTC z zyskiem 20 000, pokazuje teraz zero, bo trzyma samą gotówkę po parze. Nic nie jest policzone źle, ale wielkość zniknęła. Miejscem na nią jest moduł PnL, który ma historię transakcji; `PnlStatement` przejął jednak tę samą, niezrealizowaną liczbę i wymaga własnego rozstrzygnięcia. **Zrobione**: portfel prowadzi rejestr rozliczonych sprzedaży (`RealisedResult`), a podsumowanie podaje `realisedProfit` z pokryciem i statusem — ta sama reguła co wszędzie, tylko liczona po tym, **co sprzedano**, a nie po tym, co się trzyma. Wpis identyfikuje się transakcją, która go wytworzyła: sprzedaż ma już id i wymyślanie drugiego dałoby dwa, które mogą się nie zgadzać. Zakup gotówką nie tworzy wpisu — oddanie euro za euro niczego nie rozlicza. Test na żywym backendzie znalazł drugi, gorszy błąd: agregat odejmował koszt od przychodu **bez patrzenia na waluty**, bo `Money.minus` zachowuje walutę lewego składnika i odejmuje same kwoty. Sprzedaż 1 uncji za 2200 EUR przy koszcie 2000 USD dawała „200 EUR" → 220 USD, zamiast 2420 − 2000 = **420 USD**. Liczba wiarygodna i fałszywa, czyli najgorszy możliwy rodzaj. Naprawione u źródła: wpis trzyma **przychód i koszt osobno**, każde w swojej walucie, a odejmowanie dzieje się po stronie odczytu, gdzie są kursy — ta sama decyzja, którą C9 podjęło dla wkładów. Przy okazji wyszło, że przy częściowym pokryciu przychód trzeba brać w tej samej proporcji, bo inaczej nieopłacone jednostki raportują się jako czysty zysk. Przy okazji wyszła luka: transakcja mogła nie mieć **żadnej daty** i nikomu to nie przeszkadzało, bo nikt jej nie czytał; teraz czyta ją wynik zrealizowany, więc `TradeRestController` uzupełnia moment zapisu, gdy klient go nie poda. | P2 | C3 |
| F11 | Indeksy nie przeżywały startu aplikacji | Znalezione przy F1 i szersze niż ono. Dwie niezależne przyczyny, każda wystarczająca, żeby **żaden indeks w projekcie nie istniał**. Po pierwsze `auto-index-creation` było wyłączone — domyślnie `false` od Spring Boota 3 — więc 28 adnotacji `@Indexed` w cashflow było komentarzem; w Boocie 4 właściwości połączenia przeniosły się do `spring.mongodb`, ale **ta jedna została** pod `spring.data.mongodb` i pod złym kluczem milczy, bez ostrzeżenia. Po drugie, nawet po włączeniu indeksy ginęły: pięć `DataCleaner`-ów robiło przy starcie `dropCollection`, a drop zabiera indeksy razem z kolekcją — powstawały z adnotacji i znikały ułamek sekundy później, a pierwszy zapis odtwarzał kolekcję gołą. Zamienione na usuwanie dokumentów: „czysty start" znaczy puste dane, nie skasowaną strukturę. **Testy tego nie widziały, kontener tak** — i to jest tu lekcja: test, który sprawdza obecność adnotacji, nie dowodzi niczego; trzeba pytać bazę (`indexOps(...).getIndexInfo()`), co robi `TradeUniquenessIndexTest`. Po poprawce: `trade` 2 indeksy, `pattern_mappings` 8, `ai_csv_transformations` 7, `staging_sessions` 6. Zapisane w `CLAUDE.md`. | P2 | — |
| F10 | Idempotencja listenerów | F1 chroni przed podwójnym **zapisem** transakcji. Nie chroni przed podwójnym **zastosowaniem** już zapisanej do portfela — gdyby `TradeCapturedEvent` albo `OrderFilledEvent` przyszedł dwa razy dla tego samego rekordu, portfel policzyłby zakup i wynik zrealizowany dwukrotnie. Potrzebny jest ślad „to zdarzenie już zostało zastosowane" po stronie portfela. Blisko `TODO-kafka-dead-letter-queue`: przy okazji F6 widziałem odrzuconą transakcję, która poszła w dziewięć ponowień i przepadła bez śladu. **Zrobione.** Portfel pamięta, które transakcje już policzył (`appliedTrades`), i powtórkę **ignoruje**, nie odrzuca — wołający prosi o coś, co już jest prawdą. Ślad leży **wewnątrz portfela**, nie w osobnej tabeli zdarzeń, bo wtedy sprawdzenie i zmiana to **jeden zapis**: rozbite na dwa dokumenty bez transakcji każda kolejność przegrywa — oznacz najpierw, a nieudane zastosowanie gubi transakcję na zawsze; zastosuj najpierw, a awaria przed oznaczeniem liczy ją dwa razy. Koszt jest jawny: zbiór tylko rośnie, więc portfel ze stoma tysiącami fillów niesie sto tysięcy identyfikatorów — to znany limit, nie przeoczenie. **Pokryte komponentowo** (powtórzony zakup liczy się raz, powtórzona sprzedaż rozlicza się raz, pamięć przeżywa zapis i odczyt). Ścieżki przez Kafkę nie da się wywołać na żądanie, więc redelivery pozostaje sprawdzone regułą, nie inscenizacją — to jest świadoma granica dowodu. | P2 | F1 |
| F1 | Indeks unikalności `OriginTradeId` | Zero indeksów w `vidulum-wealth`; OKX powtarza komunikaty, Kafka ma redelivery. **Zrobione.** Klucz to `(portfolioId, originTradeId)`, nie sam identyfikator: dwie giełdy mogą wydać ten sam ciąg, a reguła globalna odrzucałaby prawdziwą transakcję drugiego właściciela. Duplikat kończy się **powodzeniem bez zmian** — wołający prosi o coś, co już jest prawdą, a błąd wysłałby tę samą wiadomość jeszcze raz w kółko. Zdarzenie przy duplikacie **nie jest emitowane**: zastosowałoby ten sam zakup do portfela drugi raz. **Znaczenie pola przepisane**, nie dodane nowe: `originTradeId` to „jak transakcja nazywa się u źródła". Dla fillu źródłem jest giełda — i ten sam ciąg niesie eksport CSV, co czyni ponowny import zachodzącego zakresu nieszkodliwym. Dla transakcji ręcznej źródłem jest klient: mintuje UUID **przy otwarciu formularza** i wysyła ten sam przy ponowieniu, i to jest jedyne, co chroni przed podwójnym kliknięciem. Gdy nie przyśle nic, backend mintuje własny — to usuwa nulle z kolumny (zwykły indeks unikalny zamiast wariantu `sparse`), ale **nie chroni nikogo** i jest tak opisane, zamiast być liczone jako idempotencja. Bez prefiksów typu `manual:`: taksonomia pilnowana tylko po jednej stronie jest gorsza niż jej brak; gdy będzie potrzebna, dojdzie jawne pole źródła. **`POST /trades` zwraca teraz zapisaną transakcję** — przy mintowaniu po stronie backendu wołający nie miał jak poznać wartości, którą powinien przysłać przy ponowieniu. **Największe znalezisko jest szersze niż zadanie**: `auto-index-creation` nie było włączone nigdzie, a od Spring Boota 3 domyślnie jest `false` — czyli **wszystkie 28 adnotacji `@Indexed` w cashflow nie tworzyło niczego**. Do tego w Boocie 4 właściwości połączenia przeniosły się do `spring.mongodb`, ale **ta jedna została** pod `spring.data.mongodb`; pod złym kluczem milczy. Złapał to dopiero test, który sprawdza, czy indeks **istnieje w bazie**, a nie czy adnotacja jest w kodzie. | P2 | — |
...
| 22 | `investedBalance` — **rozstrzygnięte**: zastąpione listą wkładów | zrobione w C9 i C12; szczegóły w [2026-09-23-invested-balance-as-contributions.md](2026-09-23-invested-balance-as-contributions.md) |
| 15 | ~~`investedBalance` zostaje zerem w POC (C9)~~ | **cofnięte** w C9/C12: zero zniknęło z odpowiedzi, a POC podaje wkład razem z pokryciem |
