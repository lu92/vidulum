# OKX → Portfolio — tablica zadań

Lista robocza do śledzenia postępu. **Ten plik jest źródłem prawdy o statusach**; uzasadnienia,
diagramy i kontekst są w [analizie](2026-09-18-okx-exchange-backend-integration.md).

Aktualizując status, zmieniaj go **tutaj**, nie w analizie.

## Legenda


| priorytet | znaczenie |
|---|---|
| **P0** | blokuje POC |
| **P1** | poprawność liczb |
| **P2** | poprawność długoterminowa |
| **P3** | dług techniczny |

| status | znaczenie |
|---|---|
| `open` | nierozpoczęte |
| `wip` | w toku |
| `blocked` | zablokowane, opisz czym |
| `finished` | zrobione |

## Postęp

| | P0 | P1 | P2 | P3 | razem |
|---|---|---|---|---|---|
| `finished` | 1 | 0 | 0 | 0 | **1** |
| `open` | 18 | 16 | 6 | 3 | **43** |
| **razem** | 19 | 16 | 6 | 3 | **44** |

## Można zacząć od razu

Zadania bez żadnych zależności — nic na nie nie czeka.

| # | prio | zadanie | odblokowuje |
|---|---|---|---|
| **A1** | P0 | Decyzja o module Maven `okx` | A2 |
| **C1** | P0 | Typ `CostBasis` | C2, C9, D1, F3 |
| **F1** | P2 | Indeks unikalności `OriginTradeId` | — |
| **F2** | P3 | `Quantity` na `BigDecimal` | — |
| **F4** | P3 | `websocket-gateway` do reaktora | — |

## Zadania według ścieżek

### A — Fundamenty modułu

| # | prio | status | zadanie | zależy od | odblokowuje |
|---|---|---|---|---|---|
| **A1** | P0 | `open` | Decyzja o module Maven `okx` | — | A2 |
| **A2** | P0 | `open` | Szkielet modułu + rejestracja w reaktorze | A1 | A3, A5, A4, B1, E2 |
| **A3** | P1 | `open` | `ErrorHttpHandler` + `ErrorCode` | A2 | — |
| **A5** | P0 | `open` | Encja `ExchangeConnection` | A2 | A6 |
| **A6** | P0 | `open` | Walidacja `perm == read_only` | A5 | — |
| **A4** | P1 | `open` | `DataCleaner` dla encji OKX | A2 | — |

### B — Broker i notowania

| # | prio | status | zadanie | zależy od | odblokowuje |
|---|---|---|---|---|---|
| **B1** | P0 | `open` | `OkxBrokerQuotationProvider` | A2 | B2 |
| **B2** | P0 | `open` | Rejestracja providera | B1 | B3, B6 |
| **B3** | P0 | `open` | Weryfikacja ścieżki publikacji | B2 | B5, B4, E5 |
| **B5** | P0 | `open` | Notowania dla gotówki | B3 | E8 |
| **B6** | P1 | `open` | Endpoint statusu giełdy | B2 | E8 |
| **B4** | P2 | `open` | Łańcuch denominacji do PLN | B3 | — |

### C — Model Portfolio i Asset

| # | prio | status | zadanie | zależy od | odblokowuje |
|---|---|---|---|---|---|
| **C1** | P0 | `open` | Typ `CostBasis` | — | C2, C9, D1, F3 |
| **C2** | P0 | `open` | Rozdzielenie pozycji po `subName` | C1 | C3, C5, C6, C7, C8, D1 |
| **C3** | P1 | `open` | Wynik tylko ze znanej części | C2 | C4 |
| **C4** | P1 | `open` | Pokrycie wyniku | C3 | — |
| **C5** | P1 | `open` | Zmiana wartości majątku | C2 | — |
| **C6** | P1 | `open` | Naprawa `AggregatedPortfolio` | C2 | — |
| **C7** | P2 | `open` | Reguła sprzedaży nieznanej części | C2 | — |
| **C9** | P1 | `open` | `investedBalance` przy tworzeniu ze spec-u | C1, D3 | — |
| **C8** | P2 | `open` | „Nieznane" jako zadanie | C2 | — |

### D — PortfolioSpec

| # | prio | status | zadanie | zależy od | odblokowuje |
|---|---|---|---|---|---|
| **D1** | P0 | `open` | Encja `PortfolioSpec` + silnik różnicy | C1, C2 | D2, D4, D7, D10, D9 |
| **D2** | P0 | `open` | Odpowiedzi użytkownika | D1 | D3, D6 |
| **D3** | P0 | `open` | Zatwierdzenie i utworzenie portfela | D2 | C9, D5, D8, D11, E4 |
| **D4** | P0 | `open` | Reguły automatycznego rozstrzygania | D1 | — |
| **D5** | P1 | `open` | Odwzorowanie locków | D3 | — |
| **D6** | P1 | `open` | Reguły nadpisywania wg proweniencji | D2 | — |
| **D7** | P2 | `open` | Obsługa zestarzałego snapshotu | D1 | — |
| **D8** | P2 | `open` | Idempotencja i brak pustych spec-ów | D3 | — |
| **D10** | P1 | `open` | TTL i stany terminalne | D1 | — |
| **D11** | P1 | `open` | `confirm` zawsze na świeżym snapshocie | D3 | — |
| **D9** | P1 | `open` | `DataCleaner` dla `PortfolioSpec` | D1 | — |

### E — POC w Node

| # | prio | status | zadanie | zależy od | odblokowuje |
|---|---|---|---|---|---|
| **E1** | P0 | **finished** | Odczyt stanu z OKX | — | — |
| **E2** | P0 | `open` | Rejestracja użytkownika + JWT | A2 | E3 |
| **E3** | P0 | `open` | Zebranie snapshotu do spec-u | E2 | E4 |
| **E8** | P0 | `open` | Publikacja notowań przed onboardingiem | B5, B6 | — |
| **E4** | P0 | `open` | Przejście ścieżki spec-u | D3, E3 | E5, E7 |
| **E5** | P1 | `open` | Publikacja notowań | B3, E4 | E6 |
| **E6** | P1 | `open` | Pętla odświeżania | E5 | — |
| **E7** | P1 | `open` | Odczyt i prezentacja wyceny | E4 | — |

### F — Dług techniczny

| # | prio | status | zadanie | zależy od | odblokowuje |
|---|---|---|---|---|---|
| **F1** | P2 | `open` | Indeks unikalności `OriginTradeId` | — | — |
| **F2** | P3 | `open` | `Quantity` na `BigDecimal` | — | — |
| **F3** | P3 | `open` | Usunięcie sentineli `Price.one`/`Price.zero` | C1 | — |
| **F4** | P3 | `open` | `websocket-gateway` do reaktora | — | — |

## Graf zależności

```mermaid
flowchart LR
    classDef p0 fill:#FFCDD2,stroke:#C62828,color:#B71C1C
    classDef p1 fill:#FFE0B2,stroke:#E65100,color:#BF360C
    classDef p2 fill:#E3F2FD,stroke:#1565C0,color:#0D47A1
    classDef p3 fill:#ECEFF1,stroke:#546E7A,color:#263238
    classDef done fill:#C8E6C9,stroke:#2E7D32,color:#1B5E20

    A1["A1 Decyzja o module Maven okx"]:::p0
    A2["A2 Szkielet modułu + rejestracja w "]:::p0
    A3["A3 ErrorHttpHandler + ErrorCode"]:::p1
    A5["A5 Encja ExchangeConnection"]:::p0
    A6["A6 Walidacja perm == read_only"]:::p0
    A4["A4 DataCleaner dla encji OKX"]:::p1
    B1["B1 OkxBrokerQuotationProvider"]:::p0
    B2["B2 Rejestracja providera"]:::p0
    B3["B3 Weryfikacja ścieżki publikacji"]:::p0
    B5["B5 Notowania dla gotówki"]:::p0
    B6["B6 Endpoint statusu giełdy"]:::p1
    B4["B4 Łańcuch denominacji do PLN"]:::p2
    C1["C1 Typ CostBasis"]:::p0
    C2["C2 Rozdzielenie pozycji po subName"]:::p0
    C3["C3 Wynik tylko ze znanej części"]:::p1
    C4["C4 Pokrycie wyniku"]:::p1
    C5["C5 Zmiana wartości majątku"]:::p1
    C6["C6 Naprawa AggregatedPortfolio"]:::p1
    C7["C7 Reguła sprzedaży nieznanej częśc"]:::p2
    C9["C9 investedBalance przy tworzeniu z"]:::p1
    C8["C8 „Nieznane jako zadanie"]:::p2
    D1["D1 Encja PortfolioSpec + silnik róż"]:::p0
    D2["D2 Odpowiedzi użytkownika"]:::p0
    D3["D3 Zatwierdzenie i utworzenie portf"]:::p0
    D4["D4 Reguły automatycznego rozstrzyga"]:::p0
    D5["D5 Odwzorowanie locków"]:::p1
    D6["D6 Reguły nadpisywania wg prowenien"]:::p1
    D7["D7 Obsługa zestarzałego snapshotu"]:::p2
    D8["D8 Idempotencja i brak pustych spec"]:::p2
    D10["D10 TTL i stany terminalne"]:::p1
    D11["D11 confirm zawsze na świeżym snapsh"]:::p1
    D9["D9 DataCleaner dla PortfolioSpec"]:::p1
    E1["E1 Odczyt stanu z OKX"]:::done
    E2["E2 Rejestracja użytkownika + JWT"]:::p0
    E3["E3 Zebranie snapshotu do spec-u"]:::p0
    E8["E8 Publikacja notowań przed onboard"]:::p0
    E4["E4 Przejście ścieżki spec-u"]:::p0
    E5["E5 Publikacja notowań"]:::p1
    E6["E6 Pętla odświeżania"]:::p1
    E7["E7 Odczyt i prezentacja wyceny"]:::p1
    F1["F1 Indeks unikalności OriginTradeId"]:::p2
    F2["F2 Quantity na BigDecimal"]:::p3
    F3["F3 Usunięcie sentineli Price.one/Pr"]:::p3
    F4["F4 websocket-gateway do reaktora"]:::p3

    A1 --> A2
    A2 --> A3
    A2 --> A5
    A5 --> A6
    A2 --> A4
    A2 --> B1
    B1 --> B2
    B2 --> B3
    B3 --> B5
    B2 --> B6
    B3 --> B4
    C1 --> C2
    C2 --> C3
    C3 --> C4
    C2 --> C5
    C2 --> C6
    C2 --> C7
    C1 --> C9
    D3 --> C9
    C2 --> C8
    C1 --> D1
    C2 --> D1
    D1 --> D2
    D2 --> D3
    D1 --> D4
    D3 --> D5
    D2 --> D6
    D1 --> D7
    D3 --> D8
    D1 --> D10
    D3 --> D11
    D1 --> D9
    A2 --> E2
    E2 --> E3
    B5 --> E8
    B6 --> E8
    D3 --> E4
    E3 --> E4
    B3 --> E5
    E4 --> E5
    E5 --> E6
    E4 --> E7
    C1 --> F3
```

Kolory odpowiadają priorytetom: czerwony **P0**, pomarańczowy **P1**, niebieski **P2**,
szary **P3**, zielony — zrobione.
